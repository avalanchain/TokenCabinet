...
type [<AllowNullLiteral>] Observable<'T> =
    ...
    abstract pipe: [<ParamArray>] operations: ResizeArray<OperatorFunction<obj option, obj option>> -> Observable<'R>
...
type [<AllowNullLiteral>] Subjects =
...
type [<AllowNullLiteral>] Callbacks =
...
type [<AllowNullLiteral>] Callbacks2 =
...
type [<AllowNullLiteral>] CombineLatests =
...
type [<AllowNullLiteral>] ObservableExports =
...
type [<AllowNullLiteral>] NodeCompatibleEventEmitter =
    abstract addListener: (string -> NodeEventHandler -> U2<unit, obj>) with get, set
    abstract removeListener: (string -> NodeEventHandler -> U2<unit, obj>) with get, set
...
type EventTargetLike<'T> =
    U4<HasEventTargetAddRemove<'T>, NodeStyleEventEmitter, NodeCompatibleEventEmitter, JQueryStyleEventEmitter>

[<RequireQualifiedAccess; CompilationRepresentation(CompilationRepresentationFlags.ModuleSuffix)>]
module EventTargetLike =
    let ofHasEventTargetAddRemove v: EventTargetLike<'T> = v |> U4.Case1
    let isHasEventTargetAddRemove (v: EventTargetLike<'T>) = match v with U4.Case1 _ -> true | _ -> false
    let asHasEventTargetAddRemove (v: EventTargetLike<'T>) = match v with U4.Case1 o -> Some o | _ -> None
    let ofNodeStyleEventEmitter v: EventTargetLike<'T> = v |> U4.Case2
    let isNodeStyleEventEmitter (v: EventTargetLike<'T>) = match v with U4.Case2 _ -> true | _ -> false
    let asNodeStyleEventEmitter (v: EventTargetLike<'T>) = match v with U4.Case2 o -> Some o | _ -> None
    let ofNodeCompatibleEventEmitter v: EventTargetLike<'T> = v |> U4.Case3
    let isNodeCompatibleEventEmitter (v: EventTargetLike<'T>) = match v with U4.Case3 _ -> true | _ -> false
    let asNodeCompatibleEventEmitter (v: EventTargetLike<'T>) = match v with U4.Case3 o -> Some o | _ -> None
    let ofJQueryStyleEventEmitter v: EventTargetLike<'T> = v |> U4.Case4
    let isJQueryStyleEventEmitter (v: EventTargetLike<'T>) = match v with U4.Case4 _ -> true | _ -> false
    let asJQueryStyleEventEmitter (v: EventTargetLike<'T>) = match v with U4.Case4 o -> Some o | _ -> None
