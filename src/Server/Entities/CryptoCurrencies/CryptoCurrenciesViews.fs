...
module Views =
  let index (ctx : HttpContext) (objs : CryptoCurrency list) =
    let cnt = [
      div [_class "container "] [
        h2 [ _class "title"] [rawText "Listing CryptoCurrencies"]
        ...
        a [_class "button is-text"; _href (Links.add ctx )] [rawText "New CryptoCurrency"]
      ]
    ]
    App.layout ([section [_class "section"] cnt])


  let show (ctx : HttpContext) (o : CryptoCurrency) =
    let cnt = [
      div [_class "container "] [
        h2 [ _class "title"] [rawText "Show CryptoCurrency"]
  ...
  let private form (ctx: HttpContext) (o: CryptoCurrency option) (validationResult : Map<string, string>) isUpdate =
  ...
  let add (ctx: HttpContext) (o: CryptoCurrency option) (validationResult : Map<string, string>)=
    form ctx o validationResult false

  let edit (ctx: HttpContext) (o: CryptoCurrency) (validationResult : Map<string, string>) =
