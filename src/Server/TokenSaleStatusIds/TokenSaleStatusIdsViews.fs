...
module Views =
  let index (ctx : HttpContext) (objs : TokenSaleStatusId list) =
    let cnt = [
      div [_class "container "] [
        h2 [ _class "title"] [rawText "Listing TokenSaleStatusIds"]
        ...
        a [_class "button is-text"; _href (Links.add ctx )] [rawText "New TokenSaleStatusId"]
      ]
    ]
    App.layout ([section [_class "section"] cnt])


  let show (ctx : HttpContext) (o : TokenSaleStatusId) =
    let cnt = [
      div [_class "container "] [
        h2 [ _class "title"] [rawText "Show TokenSaleStatusId"]
  ...
  let private form (ctx: HttpContext) (o: TokenSaleStatusId option) (validationResult : Map<string, string>) isUpdate =
  ...
  let add (ctx: HttpContext) (o: TokenSaleStatusId option) (validationResult : Map<string, string>)=
    form ctx o validationResult false

  let edit (ctx: HttpContext) (o: TokenSaleStatusId) (validationResult : Map<string, string>) =
