...
module Views =
  let index (ctx : HttpContext) (objs : TokenSaleStageStatus list) =
    let cnt = [
      div [_class "container "] [
        h2 [ _class "title"] [rawText "Listing TokenSaleStageStatuses"]

        table [_class "table is-hoverable is-fullwidth"] [
          thead [] [
            tr [] [
              th [] [rawText "Id"]
              th [] [rawText "TokenSaleStageId"]
              th [] [rawText "Status"]
          ...
          tbody [] [
            for o in objs do
              yield tr [] [
                ...
                td [] [rawText (string o.TokenSaleStageId)]
        ...
        a [_class "button is-text"; _href (Links.add ctx )] [rawText "New TokenSaleStageStatus"]
      ]
    ]
    App.layout ([section [_class "section"] cnt])


  let show (ctx : HttpContext) (o : TokenSaleStageStatus) =
    let cnt = [
      div [_class "container "] [
        h2 [ _class "title"] [rawText "Show TokenSaleStageStatus"]

        ul [] [
          li [] [ strong [] [rawText "Id: "]; rawText (string o.Id) ]
          li [] [ strong [] [rawText "TokenSaleStageId: "]; rawText (string o.TokenSaleStageId) ]
  ...
  let private form (ctx: HttpContext) (o: TokenSaleStageStatus option) (validationResult : Map<string, string>) isUpdate =
    ...
    let cnt = [
      div [_class "container "] [
        form [ _action (if isUpdate then Links.withId ctx (string o.Value.Id) else Links.index ctx ); _method "post"] [
          ...
          yield field (fun i -> (string i.TokenSaleStageId)) "TokenSaleStageId" "TokenSaleStageId" 
  ...
  let add (ctx: HttpContext) (o: TokenSaleStageStatus option) (validationResult : Map<string, string>)=
    form ctx o validationResult false

  let edit (ctx: HttpContext) (o: TokenSaleStageStatus) (validationResult : Map<string, string>) =
