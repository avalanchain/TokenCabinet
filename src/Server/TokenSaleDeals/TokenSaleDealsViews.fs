...
module Views =
  let index (ctx : HttpContext) (objs : TokenSaleDeal list) =
    let cnt = [
      div [_class "container "] [
        h2 [ _class "title"] [rawText "Listing TokenSaleDeals"]
        ...
        a [_class "button is-text"; _href (Links.add ctx )] [rawText "New TokenSaleDeal"]
      ]
    ]
    App.layout ([section [_class "section"] cnt])


  let show (ctx : HttpContext) (o : TokenSaleDeal) =
    let cnt = [
      div [_class "container "] [
        h2 [ _class "title"] [rawText "Show TokenSaleDeal"]
  ...
  let private form (ctx: HttpContext) (o: TokenSaleDeal option) (validationResult : Map<string, string>) isUpdate =
  ...
  let add (ctx: HttpContext) (o: TokenSaleDeal option) (validationResult : Map<string, string>)=
    form ctx o validationResult false

  let edit (ctx: HttpContext) (o: TokenSaleDeal) (validationResult : Map<string, string>) =
